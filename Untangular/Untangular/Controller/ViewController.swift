...
class ViewController: UIViewController {
    ...
    var currentLevel = 0
    private var connections = [ConnectionView]()
    let renderLines = UIImageView()
    override func viewDidLoad() {
        super.viewDidLoad()
        // Do any additional setup after loading the view.
        //        self.view.backgroundColor = .black.withAlphaComponent(0.8)
        
        renderLines.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(renderLines)
        
        NSLayoutConstraint.activate([
            renderLines.topAnchor.constraint(equalTo: view.topAnchor),
            renderLines.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            renderLines.trailingAnchor.constraint(equalTo: view.trailingAnchor),
            renderLines.leadingAnchor.constraint(equalTo: view.leadingAnchor)
        ])
        levelUp()
    ...
    private func place(_ connection: ConnectionView) {
        let randomX = CGFloat.random(in: 20...view.bounds.width - 20)
        let randomY = CGFloat.random(in: 100...view.bounds.height - 100)
        connection.center = CGPoint(x: randomX, y: randomY)
        
    }
    
    //    MARK: Core Graphics function
    func redrawLines() {
        let renderer = UIGraphicsImageRenderer(bounds: view.bounds)
        renderLines.image = renderer.image { context in
